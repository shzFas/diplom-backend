...
уроки и оценки с аудитом. Школу можно провести через полный цикл: завести
год и четверти, классы, предметы и назначения, зачислить учеников,
запланировать уроки и выставить оценки — с историей правок.

**Все восемь приёмочных критериев из `docs/permissions.md` закрыты.**
Осталось: посещаемость, уведомления в Telegram и расчёт итоговых
за четверть — последний заблокирован отсутствием нормативных
коэффициентов (см. «Дальше»).
...
| Интеграционные тесты | 109 тестов против PostgreSQL в Testcontainers |
...
числился **на дату урока**. Уроки и оценки пишет только `TEACHER` и только
в своих назначениях: завуч журнал читает, но не ведёт и не правит —
иначе аудит правок потерял бы смысл.
...
~~grades~~ → **attendance** → notifications. Приёмочные критерии в конце
`docs/permissions.md` — все восемь закрыты.

**Посещаемость** — небольшой модуль на тех же предикатах владения:
`PUT /lessons/{id}/attendance` списком и `GET /students/{id}/attendance`.
Четыре статуса вместо булева `markFalse`, который не отличал болезнь
от прогула (правила D10, D11).
