...
**Оценки.** Писать их может только `TEACHER`, и только в своих
назначениях (правило D8). `ADMIN` намеренно не имеет права записи: правка
оценок — исключительно учительское действие, и обход этого правила
административной ролью сделал бы аудит бессмысленным. Ученик читает только
свои оценки; журнал урока ему не показывается вовсе.

`PUT /lessons/{id}/grades` идемпотентен в строгом смысле: повторная
отправка того же тела не создаёт записей аудита. Балл, совпавший с уже
выставленным, не порождает `UPDATED` — иначе аудит забился бы шумом от
повторных нажатий «сохранить».

Отсутствующие в теле ученики **не** теряют оценки. Строгая семантика PUT
потребовала бы их удалить, но удаление требует причины, которая попадает
в аудит, а в теле `PUT` её взять негде. Удаление оформляется отдельным
`DELETE /grades/{id}` с обязательным `reason`.

Журнал применяется целиком или никак: если хотя бы один ученик не числился
в классе на дату урока (правило D9), отклоняется весь запрос с перечислением
таких учеников в `details`. Частично применённый журнал хуже отклонённого —
учитель не увидел бы, что часть класса не прошла.

`GET /grades/{id}/audit` работает и для уже удалённой оценки: `grade_audit.grade_id`
объявлен без внешнего ключа именно для этого (правило D17). Принадлежность
такой записи определяется по самому аудиту — учитель видит историю тех
оценок, которые правил сам.
