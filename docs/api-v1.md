...
GET    /lessons/{id}/attendance                   → ведомость по уроку
...
**Посещаемость.** Отдельный домен от оценок (правило D10): возможны все
четыре сочетания — отсутствовал без оценки, отсутствовал и получил балл
за отработку, присутствовал без оценки, присутствовал с оценкой. Одна
сущность не выводится из другой.

Четыре статуса вместо булева `markFalse`: `PRESENT`, `EXCUSED`,
`UNEXCUSED`, `LATE`. Школа отчитывается различием между болезнью
и прогулом, поэтому оно в типе, а не в комментарии.

`GET /lessons/{id}/attendance` — дополнение к первой редакции контракта:
она описывала только запись, а без чтения ведомость непригодна — учитель
не видел бы, кого уже отметил. Неотмеченный ученик приходит со
`status: null`, что не то же самое, что присутствовавший.

Права те же, что у оценок: ведёт только `TEACHER` в своих назначениях,
`ADMIN` читает, ученик видит только свою посещаемость. Аудита у отметок
нет — схема его не предусматривает, и в отличие от оценки отметка
не является юридически значимой записью.
